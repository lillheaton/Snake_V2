...
using Snake.Definitions.Packages;
using System;
...
namespace Server
{
    public class NetworkServerManager
    {
                ...
                while ((this.IncomingPackage = Server.ReadMessage()) != null)
                {
                    switch (this.IncomingPackage.MessageType)
                    {
                        case NetIncomingMessageType.ConnectionApproval:
                            ...
                            // Raise event
                            this.OnNewConnection(IncomingPackage.SenderConnection);
        ...
        public void Send(NetConnection recipient, IBasePackage package)
        {
            this.Server.SendMessage(package.Encrypt(this.Server), recipient, NetDeliveryMethod.ReliableOrdered);
        }
